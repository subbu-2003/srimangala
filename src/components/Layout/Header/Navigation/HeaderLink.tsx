...
const HeaderLink: React.FC<{ item: HeaderItem }> = ({ item }) => {
  ...
  return (
    ...
    >
      <Link href={item.href} className={`text-base text-black py-3 dark:text-black flex font-normal hover:text-black dark:hover:text-black ${path === item.href ? 'text-black dark:!text-black' : ' text-black dark:text-black '} ${path.startsWith(`/${item.label.toLowerCase()}`) ? 'text-primary dark:!text-primary' : null}`}>
        {item.label}
        {item.submenu && (
